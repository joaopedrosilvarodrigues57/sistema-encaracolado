...
media = [] # cria uma lista para armazenar as médias anuais que serão usadas no cálculo do desvio padrão
mmf = [] #define uma lista para poder subtrair a média de toda
mmf1 = []#define uma lista para elevar os valores da lista mmf ao quadrado
somatoriammf1 = [] #define uma lista para aramazenar a soma de todos os valores da lista mmf2
...
dpff = [] #cria uma lista que armazena o resultado final do desvio padrão e multiplica por 2 para separar as vazões acima e abaixo da média depois
...
for l in lista1: #for que percorre a lista1
    mmf.append(l - media[0]) #subtrai o valor da média anual de todos os valores na lista1 como parte do cálculo do desvio padrão e joga o resultado na lista mmf2
for l in mmf: #for que percorre a lista mmf2
    mmf1.append(l**2) #eleva os valoreds na lista mmf2 e acrescenta o resultado na lista mmf22
somatoriammf1 =[sum(mmf1)] #lista que recebe a soma de todos os valores na lista mmf22
dp = [float(i)/len(lista1)-1 for i in somatoriammf1] #divide todos os valores na lista anterior e joga o resultado na lista dp2
for n in dp: #for que percorre a lista dp2
    if n > 0: #analisa se os valores  na lista dp2 são maiores que 0
        dpf = [math.sqrt(dp[0])] #se forem maiores que 0, o loop retira a raiz quadrada desses valores e joga o resultado na lista dpf2 
    if n < 0:#analisa se o valor na lista dp2 são menores que 0
...
for i in lista1:          # laço de repetição para percorrer as colunas e separar os dias das vazões acima e abaixo da média.
    if i > media[0] + dpff2[0]: # condicional para separar os dias de vazão acima da média.
        ...
        vb +=1      # armazena a quantidade de dias que a vazão foi menor que média.
